...
from App_admin.views import is_customer
...
def user_profile_view(request):
    my_chat_room = ChatRoom.objects.get(name=request.user.username)
    try:
...
def customer_chat_room(request):
    ...
    return render(request, 'App_main/all_chat_room_customer.html', context=content)


@login_required(login_url='App_admin:admin-login-system')
@user_passes_test(is_customer)
def customer_messages(request, pk):
    ...
    return render(request, 'App_main/all_messages_customer.html', context=content)


def customer_get_messages(request, room):
...
def customer_message_send(request):
    if request.method == 'POST':
